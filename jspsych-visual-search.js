...
jsPsych.plugins["visual-search"] = (function() {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      num_stimuli: {
        ...
        default: undefined,
        description: 'How many stimuli to show.'
      }
    }
  }
  ...
  plugin.trial = function(display_element, trial) {
    ...
    /*for (i = 0; i < trial.set_size - trial.num_stimuli; i++) {
      display_locs.pop()
    }*/
    for(i = display_locs.length-1;i>=trial.num_stimuli;i--){
      display_locs.splice(Math.floor(Math.random()*display_locs.length), 1);
    }
    ...
    if(!Array.isArray(trial.foil)){
      ...
      if (trial.target_present) {
        for(var i=0; i<trial.num_stimuli - 1; i++){
          fa.push(trial.foil);
        }
      }
      else {
        for(var i=0; i<trial.num_stimuli; i++){
          fa.push(trial.foil);
        }
      }
      trial.foil = fa;
